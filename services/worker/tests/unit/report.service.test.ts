...
describe('Report Service', () => {
  ...
  describe('validateCustomDateRange', () => {
    it('should validate correct date range', () => {
      const result = reportService.validateCustomDateRange('2025-01-01', '2025-01-31');
    ...
    it('should reject invalid dates', () => {
      const result = reportService.validateCustomDateRange('2025-13-01', '2025-01-31');
    ...
    it('should reject start date after end date', () => {
      const result = reportService.validateCustomDateRange('2025-02-01', '2025-01-31');
    ...
    it('should reject future dates', () => {
      ...
      const result = reportService.validateCustomDateRange('2026-01-01', futureDateStr);

      expect(result.valid).toBe(false);
      expect(result.error).toBe('future_date');
    });

    it('should reject date range too large (>2 years)', () => {
      const result = reportService.validateCustomDateRange('2020-01-01', '2023-01-01');

      expect(result.valid).toBe(false);
      expect(result.error).toBe('range_too_large');
    });

    it('should reject dates before year 2000 (when no userEarliestDate)', () => {
      const result = reportService.validateCustomDateRange('1999-01-01', '1999-12-31');

      expect(result.valid).toBe(false);
      expect(result.error).toBe('date_too_old');
    });

    it('should accept date range up to today', () => {
      const today = new Date().toISOString().split('T')[0];
      const lastMonth = new Date();
      lastMonth.setMonth(lastMonth.getMonth() - 1);
      const lastMonthStr = lastMonth.toISOString().split('T')[0];

      const result = reportService.validateCustomDateRange(lastMonthStr, today);

      expect(result.valid).toBe(true);
    });

    it('should reject dates before user first invoice date', () => {
      const userEarliestDate = '2025-06-01';
      const result = reportService.validateCustomDateRange(
        '2025-05-01',
        '2025-05-31',
        userEarliestDate
      );
      ...
      expect(result.valid).toBe(false);
      expect(result.error).toBe('before_first_invoice');
      expect(result.earliestAllowed).toBe('2025-06-01');
    });

    it('should accept dates on or after user first invoice date', () => {
      const userEarliestDate = '2025-06-01';
      const result = reportService.validateCustomDateRange(
        '2025-06-01',
        '2025-06-30',
    ...
    it('should accept dates after user first invoice date', () => {
      const userEarliestDate = '2025-01-15';
      const result = reportService.validateCustomDateRange(
        '2025-06-01',
        '2025-06-30',
        userEarliestDate
